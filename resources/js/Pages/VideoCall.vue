<template>
    ...
    <AuthenticatedLayout>
        <div class="relative h-screen bg-gray-900 text-white overflow-hidden">
            ...
            <div
                class="absolute bottom-4 right-4 w-1/4 border-2 border-white bg-black"
            >
                <video
                    ref="localVideo"
                    autoplay
                ...
                <div
                    v-if="cameraOff"
                    class="absolute inset-0 bg-black/60 flex items-center justify-center text-sm"
                >
                    Camera Off
                </div>
            </div>
            <div
                class="absolute bottom-0 left-0 right-0 flex justify-center space-x-2 p-4 bg-black/50"
            >
                <select
                ...
                <button
                    @click="toggleMic"
                    class="p-2 rounded-full bg-gray-200 text-black hover:bg-gray-300"
                >
                    <svg
                        v-if="!isMuted"
                        class="h-6 w-6"
    ...
    const constraints = {
        audio: {
            deviceId: selectedAudio.value
                ? { exact: selectedAudio.value }
                : undefined,
        },
        video: {
            deviceId: selectedVideo.value
                ? { exact: selectedVideo.value }
                : undefined,
        },
    };
